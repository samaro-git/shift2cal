(function () {
  ...
  if (btnDownloadAll) {
    btnDownloadAll.addEventListener('click', function () {
      if (!state.currentEvents || !state.currentEvents.length) return;
      var sel = getSelectedYearMonth();
      var name = (state.selectedName || nameSelect.value || 'Dienstplan').replace(/\s+/g, '_');
      var filename = 'Dienstplan_' + name + '_' + sel.year + '-' + String(sel.month).padStart(2, '0') + '.ics';
      download(buildICS(state.currentEvents), filename);
    });
  }
